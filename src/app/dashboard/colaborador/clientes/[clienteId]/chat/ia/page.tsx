...
export default function ChatIaColab() {
  ...
  useEffect(() => {
    ...
    async function loadChat() {
      try {
        ...
        if (!c) throw new Error('Chat do tipo "ia" não encontrado')
        ...
        setError(`Falha ao preparar o chat com IA: ${e.message}`)
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSendMessage = async (content: string) => {
    ...
    // A mensagem enviada será recebida de volta via WebSocket
  }
